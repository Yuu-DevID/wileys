        ...
        case WAProto_1.proto.HistorySync.HistorySyncType.ON_DEMAND:
            for (const chat of item.conversations) {
                ...
                for (const item of msgs) {
                    ...
                    // [FIX-HISTORY-PARTICIPANT] WA history sync proto meletakkan participant
                    // di root message (bukan di key). Pindahkan ke key.participant supaya
                    // handler downstream bisa membaca sender dengan benar.
                    // Juga resolve @lid -> @s.whatsapp.net.
                    if (message.key && (0, WABinary_1.isJidGroup)(message.key.remoteJid)) {
                        // Ambil participant: prioritaskan root-level, fallback ke key-level
                        const rootParticipant = message.participant;
                        const keyParticipant = message.key.participant;
                        let resolvedParticipant = rootParticipant || keyParticipant;
                        if (resolvedParticipant) {
                            // Resolve @lid -> @s.whatsapp.net
                            if ((0, WABinary_1.isLidUser)(resolvedParticipant)) {
                                resolvedParticipant = (0, WABinary_1.lidToJid)(resolvedParticipant);
                            }
                            // Jangan pakai group JID sendiri sebagai participant
                            if ((0, WABinary_1.isJidGroup)(resolvedParticipant)) {
                                resolvedParticipant = undefined;
                            }
                        }
                        message.key.participant = resolvedParticipant;
                        // Hapus dari root agar tidak membingungkan downstream
