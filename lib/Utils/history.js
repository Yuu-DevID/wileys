        ...
        case WAProto_1.proto.HistorySync.HistorySyncType.ON_DEMAND:
            for (const chat of item.conversations) {
                ...
                for (const item of msgs) {
                    ...
                    // Fix participant field: if participant is @g.us (same as remoteJid) or @lid, resolve to @s.whatsapp.net
                    if (message.key) {
                        const { remoteJid, participant } = message.key;
                        if (participant) {
                            if ((0, WABinary_1.isLidUser)(participant)) {
                                // Convert @lid -> @s.whatsapp.net
                                message.key.participant = (0, WABinary_1.lidToJid)(participant);
                            }
                            else if ((0, WABinary_1.isJidGroup)(participant) && participant === remoteJid) {
                                // participant is wrongly set to the group JID itself — clear it so
                                // the actual sender identity isn't masked by the group JID
