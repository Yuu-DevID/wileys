...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, keyStore, logger, options, getMessage }) => {
    ...
    if ((0, WABinary_1.isLidUser)(senderId)) {
        // [FIX-LID] lidToJid() now uses the phone map, not naive string replacement.
        // Only override if we have a real mapping — otherwise keep @lid so callers
        // (e.g. history sync in chats.js) can handle the unknown sender gracefully.
        ...
        // If lidToJid returns null, keep the @lid — do NOT do a naive @s.whatsapp.net swap.
    }
