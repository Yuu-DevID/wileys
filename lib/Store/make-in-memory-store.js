...
exports.default = (config) => {
    ...
    const bind = (ev) => {
        ...
        ev.on('messaging-history.set', ({ chats: newChats, contacts: newContacts, messages: newMessages, isLatest, syncType }) => {
            ...
            // [FIX-LID] Update global lid→phone map from synced contacts before storing chats/messages
            // Contacts from history sync carry { id: phoneJid, lid: lidJid } or { id, jid, lid } fields
            if (newContacts && newContacts.length > 0) {
                (0, WABinary_1.initLidPhoneMap)(newContacts);
            }
        ...
        ev.on('contacts.upsert', contacts => {
            // [FIX-LID] Keep lid map current as new contacts stream in
            if (contacts && contacts.length > 0) {
                (0, WABinary_1.initLidPhoneMap)(contacts);
            }
            contactsUpsert(contacts);
        });
        // [FIX-LID] SHARE_PHONE_NUMBER protocol message reveals the real phone JID for a @lid
        // Update the map immediately so subsequent messages can be resolved.
        ev.on('chats.phoneNumberShare', ({ lid, jid }) => {
            if (lid && jid) {
                (0, WABinary_1.updateLidPhoneMap)(lid, jid);
            }
        });
