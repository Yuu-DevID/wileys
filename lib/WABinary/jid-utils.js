...
// ── LID ↔ Phone JID resolution map ──────────────────────────────────────────
// Keyed by lidUser (bare number before @lid). Value = full @s.whatsapp.net JID.
// Populated via initLidPhoneMap() / updateLidPhoneMap() called from history sync
// and contacts.upsert events. NOT populated by naive string replacement.
const _lidPhoneMap = new Map();

/**
 * initLidPhoneMap — seed the global map from an array of { lid, jid } pairs
 * or from contacts array with { id, lid } fields.
 * Called once after history sync contacts are processed.
 */
const initLidPhoneMap = (entries) => {
    for (const entry of entries) {
        // entries can be { lid: '123@lid', jid: '62x@s.whatsapp.net' }
        // or { id: '62x@s.whatsapp.net', lid: '123@lid' }
        const lidRaw  = entry.lid || null;
        const pnRaw   = entry.jid || (entry.id && !entry.id.endsWith('@lid') ? entry.id : null);
        if (!lidRaw || !pnRaw) continue;
        const lidUser = lidRaw.split('@')[0];
        const pnJid   = pnRaw.endsWith('@s.whatsapp.net') ? pnRaw : `${pnRaw.split('@')[0]}@s.whatsapp.net`;
        if (lidUser && pnJid) {
            _lidPhoneMap.set(lidUser, pnJid);
            _lidPhoneMap.set(`${lidUser}@lid`, pnJid);
        }
    }
};
exports.initLidPhoneMap = initLidPhoneMap;

/**
 * updateLidPhoneMap — add / overwrite a single lid→phone mapping.
 * Use from contacts.upsert / chats.phoneNumberShare events.
 */
const updateLidPhoneMap = (lidJid, phoneJid) => {
    if (!lidJid || !phoneJid) return;
    const lidUser = lidJid.split('@')[0];
    const pnJid   = phoneJid.endsWith('@s.whatsapp.net') ? phoneJid : `${phoneJid.split('@')[0]}@s.whatsapp.net`;
    _lidPhoneMap.set(lidUser, pnJid);
    _lidPhoneMap.set(`${lidUser}@lid`, pnJid);
};
exports.updateLidPhoneMap = updateLidPhoneMap;

/**
 * getLidPhoneMap — expose internal map for callers that want to snapshot it.
 */
const getLidPhoneMap = () => _lidPhoneMap;
exports.getLidPhoneMap = getLidPhoneMap;

/**
 * lidToJid — resolve a @lid JID → real @s.whatsapp.net JID using the phone map.
 *
 * IMPORTANT: This NEVER does a naive @lid → @s.whatsapp.net string replacement
 * because LID user IDs are NOT phone numbers. Naively replacing the server part
 * produces a fake JID (e.g. 259508803@s.whatsapp.net) that creates duplicate
 * chats in the UI.
 *
 * Returns null (not the original @lid) when the mapping is unknown so callers
 * can decide whether to retain the @lid or skip the message entirely.
 *
 * @param {string} jid
 * @returns {string|null}  real @s.whatsapp.net JID, or null if not resolved
 */
const lidToJid = (jid) => {
    if (!jid) return null;
    if (!jid.endsWith('@lid')) return jid;  // not a lid — pass through unchanged
    const resolved = _lidPhoneMap.get(jid) || _lidPhoneMap.get(jid.split('@')[0]);
    return resolved || null;  // null = unknown, caller must handle
};
